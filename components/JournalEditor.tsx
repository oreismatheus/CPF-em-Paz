...
export const JournalEditor: React.FC<JournalEditorProps> = ({
  ...
  return (
    <div className="bg-[#0a0a0a] rounded-[2.5rem] p-8 border border-[#1c1c1c] flex flex-col h-full shadow-2xl">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 gap-4">
        ...
        <input
          ...
          className="bg-[#000000] border border-[#1c1c1c] text-[#ff3d00] rounded-2xl px-4 py-2 text-sm font-black focus:ring-1 focus:ring-[#ff3d00] outline-none"
        />
      </div>
      ...
      <div className="space-y-8 flex-grow">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          <div>
            ...
            <div className="flex justify-between bg-[#000000] p-2 rounded-[1.5rem] border border-[#1c1c1c]">
              {[1, 2, 3, 4, 5].map((lvl) => (
                <button
                  key={lvl}
                  ...
                  title={MOOD_LABELS[lvl-1]}
                >
                  {MOOD_EMOJIS[lvl-1]}
                </button>
              ))}
            </div>
            <p className="text-center text-[10px] text-[#ff3d00] mt-2 font-black tracking-widest">{MOOD_LABELS[mood-1]}</p>
          </div>
            ...
            <div className="flex justify-between bg-[#000000] p-2 rounded-[1.5rem] border border-[#1c1c1c]">
              {[1, 2, 3, 4, 5].map((lvl) => (
                <button
                  ...
                  title={WEATHER_LABELS[lvl-1]}
                >
                  {WEATHER_ICONS[lvl-1]}
                </button>
              ))}
            </div>
            <p className="text-center text-[10px] text-[#ff3d00] mt-2 font-black tracking-widest">{WEATHER_LABELS[weather-1]}</p>
          </div>
        </div>
        ...
        <div className="flex-grow flex flex-col min-h-[350px]">
          ...
          <textarea
            ...
            placeholder="Escreva seus planos hoje..."
            className="w-full flex-grow p-6 rounded-[2rem] bg-[#000000] border border-[#1c1c1c] focus:border-[#ff3d00]/40 outline-none resize-none text-slate-100 placeholder:text-slate-800 text-lg leading-relaxed font-medium"
          />
        </div>
      </div>
